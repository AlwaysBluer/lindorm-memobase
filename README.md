...
LindormMemobase 支持按子主题配置合并阈值，实现成本优化和性能平衡：

```python
from lindormmemobase import ProfileConfig

# 配置不同子主题的合并阈值
project_config = ProfileConfig(
    language="zh",
    merge_thresholds={
        "preferences::dietary": 1,      # 立即合并（时间敏感）
        "interests::hobbies": 10,        # 批量合并（成本优化）
        "demographics::basic_info": 50,  # 大批量合并（稳定数据）
    },
    max_pending_profiles=1000  # 每个子主题最大待合并数量
)

# 为项目设置配置
await memobase.set_project_config("my_app", project_config)

# 正常提取 - 系统自动根据阈值决定立即合并或缓存
result = await memobase.extract_memories(
    user_id="user123",
    blobs=[conversation_blob],
    project_id="my_app"
...
print(f"合并了 {merge_result.merged_count} 个待合并画像")
```
