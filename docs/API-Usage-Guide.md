...
class MultiUserMemorySystem:
    ...
    def __init__(self):
        ...
        self.user_sessions: Dict[str, List[OpenAICompatibleMessage]] = {}
    ...
    async def start(self):
        ...
        asyncio.create_task(self._memory_extraction_worker())
        print("记忆提取服务已启动")
    
    async def stop(self):
        """停止服务"""
        self.is_running = False
        await self.extraction_queue.join()
        print("记忆提取服务已停止")
    ...
    async def _memory_extraction_worker(self):
        ...
        while self.is_running:
            try:
                ...
                print(f"[{datetime.now()}] 用户 {user_id} 记忆提取完成")
    ...
    async def add_message(self, user_id: str, role: str, content: str):
        ...
        # 每4条消息触发记忆提取
        if len(self.user_sessions[user_id]) >= 4:
            blob = ChatBlob(messages=self.user_sessions[user_id][-4:])
    ...
    async def get_user_context(self, user_id: str, query: str) -> str:
        ...
        if user_id in self.user_sessions:
            conversation = self.user_sessions[user_id][-3:] + conversation
    ...
    async def get_user_stats(self, user_id: str) -> Dict:
        ...
        stats = {
            "user_id": user_id,
            "profile_topics": len(profiles),
            "total_subtopics": sum(len(p.subtopics) for p in profiles),
            "recent_events": len(events),
            "active_session": user_id in self.user_sessions
        }
        
        return stats
    
    async def search_across_users(self, query: str, user_ids: List[str]) -> Dict[str, List]:
        """跨用户搜索"""
        results = {}
        ...
        for user_id in user_ids:
            try:
                ...
                if events:
                    results[user_id] = events
                    
            except Exception as e:
                print(f"搜索用户 {user_id} 失败: {e}")
...
async def simulate_multi_user_chat():
    ...
    users_data = {
        ...
        "bob": [
            ("user", "我是Bob，程序员"),
            ("assistant", "你好Bob！"),
            ("user", "我在学习Python和机器学习"),
            ("assistant", "这是很有前景的技术领域")
        ],
        "charlie": [
            ("user", "我是Charlie，喜欢运动"),
            ("assistant", "运动很健康！"),
    ...
    for user_id, messages in users_data.items():
        for role, content in messages:
            await system.add_message(user_id, role, content)
    ...
    for user_id in users_data.keys():
        stats = await system.get_user_stats(user_id)
        print(f"{user_id}: {stats}")
    
    # 跨用户搜索
    print("\n=== 搜索 '爱好' ===")
    search_results = await system.search_across_users(
        query="爱好",
        user_ids=list(users_data.keys())
    )
    
    for user_id, events in search_results.items():
        print(f"{user_id}:")
        for event in events:
...
if __name__ == "__main__":
    asyncio.run(simulate_multi_user_chat())
```
...
- 根据需求调整 `max_token_size` 参数
- 使用 `profile_event_ratio` 平衡档案和事件内容

### 5. 搜索优化

- 设置合适的 `similarity_threshold` 避免低相关度结果
- 限制搜索结果数量 (`limit` 参数)
...
3. **搜索无结果**
   - 确认已启用嵌入功能 (`enable_event_embedding=true`)
   - 检查搜索索引是否创建
   - 调整相似度阈值
...
LindormMemobase 提供了完整的记忆管理解决方案，适用于各种 LLM 应用场景。通过合理使用 API，可以构建具有长期记忆能力的智能应用。
...
更多信息请参考项目 [GitHub 仓库](https://github.com/your-repo/lindorm-memobase) 和示例代码。