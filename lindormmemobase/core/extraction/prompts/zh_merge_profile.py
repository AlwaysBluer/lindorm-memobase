...
def get_prompt_json_mode(config=None) -> str:
    """获取 JSON Mode 提示词（用于 merge_profile）。

    返回一个指示 LLM 仅输出 JSON 的提示词，
    与 response_format={"type": "json_object"} 兼容。
    """
    return """你是备忘录维护专家。你的任务是将新信息与现有用户备忘录合并。
...
有效的 action 值：
- "APPEND"：新信息增加价值 → memo 设为 "APPEND"
- "UPDATE"：新信息冲突或需整合 → memo 设为完整重写内容
- "ABORT"：新信息冗余或无效 → memo 设为 "ABORT"
...
3. 不编造输入中未提及的内容
4. 更新时去除冗余（如："用户很伤心; 用户心情不好" → "用户很伤心"）
5. 仅返回 JSON 对象，不包含其他文字

示例：
```json
{{
  "action": "UPDATE",
  "memo": "使用多邻国自学日语，目标通过N2 [提及于 2025/05/05]；准备期末考试 [提及于 2025/06/01]"
}}
```
